...
public class Oneal extends Enemy {
    // Tốc độ animation: Số frame update cho mỗi sprite của Oneal
     ...
     * @param nesSheet SpriteSheet chứa hình ảnh của Oneal.
    ...
    private void setSpriteBasedOnDirectionAndFrame() {
        ...
        // Oneal chỉ có animation trái/phải.
        ...
        if (this.sprite == null) {
            System.err.println("Warning: Oneal sprite is null for direction " + currentDirection +
                    ", frame " + animationFrameIndex + ". Defaulting to left1.");
            this.sprite = Sprite.enemy_ballom_left1; // Fallback cuối cùng
            if (this.sprite == null) {
                System.err.println("CRITICAL ERROR: Default Oneal sprite (enemy_ballom_left1) is also null!");
            }
        }
    }

    /**
     * Ghi đè die() để sử dụng animation chết cụ thể của Oneal hoặc mob_dead chung.
     */
    @Override
    public void destroy() {
        if (isAlive() && !isDying()) { // Kiểm tra cả isDying từ Enemy
            // System.out.println("Oneal is dying!");
