            ...
            for (int x = 0; x < width; x++) {
                ...
                switch (tileChar) {
                    ...
                    case '2':
                        ...
                        mapData[y][x] = ' ';
                        break;
                    case 'x': // Ký tự cho Portal
                        if (game.getPortal() == null) { // Chỉ tạo một Portal mỗi level
                            // Giả sử sprite portal nằm trên nesSheet (SpriteSheet.Sheet2)
                            // Truyền nesSheet (hoặc sheet chứa sprite portal) vào constructor Portal
                            game.addPortal(new Portal(x, y, nesSheet));
                        } else {
                            System.err.println("Warning: Multiple portal ('x') positions found. Using the first one.");
                        }
                        mapData[y][x] = ' '; // Sau khi tạo Portal, ô đó trở thành nền cỏ
                        break;
